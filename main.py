...
MODULE      : main.py
VERSION     : 2026.Q3.v4.4.0 (Production Fail-Closed Supervisor)
PYTHON      : 3.11+
================================================================================
"""
...
class AtomicInstanceLock:
    """Process lock atomis berbasis POSIX O_CREAT | O_EXCL dengan pemulihan stale PID."""
...
class FXPAutonomousOrchestrator:
    ...
    def __init__(self, runtime: RuntimeMode = RuntimeMode.AUTO, interval: Optional[int] = None) -> None:
        ...
        if self.oanda_env == OandaEnvironment.UNKNOWN:
            logger.critical("CRITICAL: Hostname OANDA_API_URL '%s' tidak valid! Program dihentikan.", raw_api_url)
            sys.exit(1)
    ...
    def _startup_log(self) -> None:
        ...
        logger.info("=" * 78)
        logger.info("FXP AUTONOMOUS TRADING ENGINE v4.4.0 (PRODUCTION FAIL-CLOSED)")
        logger.info("=" * 78)
