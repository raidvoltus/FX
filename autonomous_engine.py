...
VERSION     : 2026.Q3.v4.4.0 (Authoritative Recovery & Idempotent Engine)
PYTHON      : 3.11+
================================================================================
"""
...
class AutonomousEngine:
    ...
    def reconcile_remote_state(self) -> Tuple[bool, str]:
        """Startup Reconciliation Total: Account, Positions, Trades, Pending Orders."""
        try:
    ...
    def run_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"Market Data Ingestion Error: {err}") from err
        ...
        for instrument in active_instruments:
            try:
                # Pre-Execution Duplicate Position & Order Check
                if hasattr(self.portfolio_manager, "has_active_position_or_order"):
                    if self.portfolio_manager.has_active_position_or_order(instrument):
                        logger.info("[%s] Diabaikan: Posisi terbuka atau pending order sudah aktif.", instrument)
                        continue
                ...
                units = self.position_sizer.calculate_units(
                    account_balance=account_balance, entry_price=entry_price, stop_loss_price=opt_sl, instrument=instrument
                )
                ...
                # Idempotent Execution ID Generation
                ...
                if status == "EXECUTED":
                    ...
                    unknown_orders.append(exec_res)
